...
def generate_injection(list_fits, ids_to_use_time, ids_to_use_time_fits=1, return_catalog=True):
...
list_fits = [sorted(glob.glob(f"fits/4026_15_v2/{subfolder}/*52000.0pc.fits"))]

my_injection_catalog_LEs = []

for llix, ids_to_use_time_fitsx in zip(list_fits, ids_to_use_time_fits):
    # print(llix)
    my_injection_catalog_LEsix, first_ct, list_metadata, coadd, wcs_b, radec, inject_size, Ns, mag_source = generate_injection(llix, ids_to_use_time, ids_to_use_time_fitsx)
...
if not_second_inj == True:
    ...
    list_fits = [sorted(glob.glob(f"fits/4026_15_v2/{subfolder_2}/*52000.0pc.fits")), sorted(glob.glob(f"fits/4026_15_v2/{subfolder}/*52000.0pc.fits"))] 
    # list_fits = [sorted(glob.glob(f"fits/4026_15_v2/{subfolder}/*52000.0pc.fits")), sorted(glob.glob(f"fits/4026_15_v2/{subfolder_2}/*52000.0pc.fits")), 
    #              sorted(glob.glob(f"fits/4026_15_v2/{subfolder}/*52000.0pc.fits"))]
    my_injection_catalog_LEs = []
    # for llix in list_fits:
    for llix, ids_to_use_time_fits_2ndx in zip(list_fits, ids_to_use_time_fits_2nd):
        my_injection_catalog_LEsix, second_ct, _, coadd, _, _, _, _, _ = generate_injection(llix, ids_to_use_time_2nd, ids_to_use_time_fits_2ndx, return_catalog=True)
        my_injection_catalog_LEs.append(my_injection_catalog_LEsix)
...
path_name_ids = f"{coadd_to_use[ids_to_use_time].dataId['tract']}_{coadd_to_use[ids_to_use_time].dataId['patch']}_{coadd_to_use[ids_to_use_time].dataId['band']}_v2"
